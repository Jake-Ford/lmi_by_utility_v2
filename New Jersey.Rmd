...
The table below shows three different proxies for LMI households in New Jersey: 80% and 100% AMI and SNAP receipient households. Totals for households and popualtion are included for context. Note, the three LMI proxies are not mutually exclusive. 
...
temp_table <- temp_tracts %>% 
  ...
  st_drop_geometry() %>%
  group_by(Utility) %>%
  summarize(`80% AMI HHs` = sum(AMI_80),
...
datatable(temp_table, caption = "New Jersey LMI Population by Utility") %>%
  formatCurrency('80% AMI HHs',currency = "", interval = 3, digits = 0, mark = ",") %>%
  formatCurrency('100% AMI HHs',currency = "", interval = 3, digits = 0, mark = ",") %>% 
