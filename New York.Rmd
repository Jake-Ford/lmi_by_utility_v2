...
```{r}
or_zips <- process_utility_data("ORU", temp_util, get_zips, pre_screened, dac_bgs)
chge_zips <- process_utility_data("CHGE", temp_util, get_zips, pre_screened, dac_bgs)
ngrid_zips <- process_utility_data("NGRID", temp_util, get_zips, pre_screened, dac_bgs)
nyseg_zips <- process_utility_data("NYSEG", temp_util, get_zips, pre_screened, dac_bgs)
...
```{r message=FALSE, warning=FALSE, include=FALSE}
#Get any zip code that is at least 50% covered by Utility Zone:
...
chge_zips <- get_zips_coverage %>%
  filter(coverage >= 0.05) # captures zips with 5% of area covered by OR Zone

pre_screened <- st_transform(pre_screened, st_crs(temp_zone)) %>%
  st_make_valid()


dac_bgs <- st_transform(dac_bgs, st_crs(temp_zone)) %>%
  st_make_valid()

pre_screened_intersects <- st_intersects(chge_zips, pre_screened, sparse = FALSE)

chge_zips$pre_screened <- apply(pre_screened_intersects, 1, any)

dac_bgs_intersects <- st_intersects(chge_zips, dac_bgs, sparse = FALSE)
chge_zips$dac <- apply(dac_bgs_intersects, 1, any)

chge_zips$Utility <- utility_name

chge_zips %>%
  st_drop_geometry() %>%
  write.csv("CHGE_Zips.csv")
...
pre_screened_intersects <- st_intersects(ngrid_zips, pre_screened, sparse = FALSE)

ngrid_zips$pre_screened <- apply(pre_screened_intersects, 1, any)

dac_bgs_intersects <- st_intersects(ngrid_zips, dac_bgs, sparse = FALSE)
ngrid_zips$dac <- apply(dac_bgs_intersects, 1, any)

ngrid_zips$Utility <- utility_name

ngrid_zips %>%
  st_drop_geometry() %>%
  write.csv("NGRID_Zips.csv")
...
pre_screened_intersects <- st_intersects(nyseg_zips, pre_screened, sparse = FALSE)

nyseg_zips$pre_screened <- apply(pre_screened_intersects, 1, any)

dac_bgs_intersects <- st_intersects(nyseg_zips, dac_bgs, sparse = FALSE)
nyseg_zips$dac <- apply(dac_bgs_intersects, 1, any)

nyseg_zips$Utility <- utility_name

nyseg_zips %>%
  st_drop_geometry() %>%
  write.csv("NYSEG_Zips.csv")
...
temp_util$area <- st_area(temp_util)
get_zips$area <- st_area(get_zips)
temp_zone$area <- st_area(temp_zone)

intersection_summary <- st_intersection(get_zips, temp_util)%>%
  mutate(intersection_area = st_area(geometry)) %>%
  st_drop_geometry() %>%
  group_by(GEOID) %>%
  summarize(intersection_area = sum(intersection_area))
...
pre_screened_intersects <- st_intersects(or_zips, pre_screened, sparse = FALSE)

or_zips$pre_screened <- apply(pre_screened_intersects, 1, any)

dac_bgs_intersects <- st_intersects(or_zips, dac_bgs, sparse = FALSE)
or_zips$dac <- apply(dac_bgs_intersects, 1, any)

or_zips$Utility <- utility_name
...
pre_screened_intersects <- st_intersects(rge_zips, pre_screened, sparse = FALSE)

rge_zips$pre_screened <- apply(pre_screened_intersects, 1, any)

dac_bgs_intersects <- st_intersects(rge_zips, dac_bgs, sparse = FALSE)
rge_zips$dac <- apply(dac_bgs_intersects, 1, any)

rge_zips$Utility <- utility_name

rge_zips %>%
  st_drop_geometry() %>%
  write.csv("RGE_Zips.csv")
...
leaflet() %>%
  ...
  addPolygons(data=or_zips,
              ...
              dashArray="3",
              fillOpacity=0.1,
              popup = paste("Name: ", or_zips$GEOID, "<br>",
                            "DAC?: ", or_zips$dac, "<br>",
                            "Pre-Screened: ", or_zips$pre_screened)) %>%
...
- goals: get zip codes covered by each utility zone - maybe using 5% coverage like we did here in NY
  - this will be the total zip codes covered by each utility zone we care about 
  - put it in a table 
- zip codes in particular UZ we care about - but a 'super' subset of zip codes with geo-lmi qualification zones
